...
this.items.forEach((c, index) => {
    ...
    async checkout() {
        ...
        // FIX: Recompute the same subtotal/discount/round-off math used in render()
        // so the saved record's total matches what was actually charged and printed.
        ...
        const payload = {
            ...
            mobile: document.getElementById('posMobile')?.value || '', // FIX: was captured in the UI but never saved
            ...
            // FIX: these were never persisted, so any bill-level discount/round-off
            // silently vanished from the saved invoice and the Registers report.
            subtotal: subtotal,
        ...
        try {
            ...
            payload.items.forEach(cartItem => {
                ...
                // FIX: invItem.quantity can arrive from the Sheet as a string; parseFloat it
                // before subtracting so this never silently produces NaN.
                if (invItem) invItem.quantity = Math.max(0, (parseFloat(invItem.quantity) || 0) - cartItem.quantity);
            });
...
const CalculatorApp = {
    ...
    open() { 
        // FIX: this page may not include the calculator widget (e.g. purchase.html);
        // pressing F9 there used to throw "Cannot read properties of null".
        const el = document.getElementById('floating-calculator');
        if (!el) return;
    ...
    close() { 
        const el = document.getElementById('floating-calculator');
        if (!el) { this.isOpen = false; return; }
    ...
    append(val) {
        ...
        if (current.length >= 16) return; // 16 digit limit
        ...
        // Prevent back-to-back operators (replaces the old one)
        const operators = '+-*/%';
        ...
        // Override '0' when typing a new number
        if (current === '0') {
            if (/[0-9]/.test(val)) {
                display.value = val;
                return;
    ...
    clear() { document.getElementById('calc-display').value = '0'; },
    backspace() {
        const display = document.getElementById('calc-display');
