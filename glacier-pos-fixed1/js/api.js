...
async getInventory() {
        ...
        try {
            ...
            // FIX: read as text first — a broken/misconfigured Apps Script deployment
            // returns HTTP 200 with an HTML error page, not JSON, which used to surface
            // as an opaque "Unexpected token '<'" SyntaxError.
            const text = await res.text();
            let data;
            try {
                data = JSON.parse(text);
            } catch {
                throw new Error('Backend did not return valid data. The Apps Script deployment URL in config.js may be outdated or misconfigured.');
            }
    ...
    async createItem(payload) {
        // FIX: this call had no timeout, so a dead/slow Apps Script backend left the
        // "Saving..." button stuck forever with no error shown. Mirrors getInventory's guard.
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 15000);
        ...
        try {
            ...
            // FIX: Apps Script returns HTTP 200 with an HTML error/login page when the
            // deployment is broken, so res.ok can be true while the body isn't JSON.
            // Read as text first and give a clear error instead of a raw "Unexpected token '<'".
            const text = await res.text();
            let data;
            try {
                data = JSON.parse(text);
            ...
            if (error.name === 'AbortError') {
                throw new Error('Save timed out. Google is slow or the deployment URL is unreachable.');
            }
            throw error;
        }
    },
        ...
        try {
            ...
            snapshot.forEach(doc => {
                ...
                registers.push({
                    ...
                    type: data.type,
                    party: data.customer,
                    // FIX: mobile/refNo was captured in the UI but previously never saved,
                    // so reprints from Registers always showed "-". Now surfaced if present.
                    mobile: data.mobile || data.refNo || '',
                    mode: data.paymentMode,
                    // FIX: use the saved total (subtotal - billDiscount + roundOff) when present.
                    // Old records saved before this fix won't have it, so fall back to summing
                    // item amounts (which is what always happened before, and is only accurate
                    // when no bill-level discount was applied).
